...
所以在实际的生产环境中，肯定是会部署一个MQ的集群。而在RocketMQ中，这个“实例”有个专属名词，叫做**Broker**。并且，每个Broker都会部署一个Slave Broker，Master Broker会定时的向Slave Broker同步数据，形成一个Broker的**主从架构**。
...
可以看到，Message的第一个参数，为当前这条消息指定了一个**Topic**，那Topic又是什么呢？
