...
const renderBirthdayList = (data) => {
    const displayContainer = document.getElementById("display-container");
    var date = getCurrentDateTime();
    var calendar = showCal();
    const birthdayInfo = `<div><p>今天是:   ${date}  农历： + ${calendar}</p><p>${birthday[0].name}</p></div>`;
    displayContainer.innerHTML = birthdayInfo;
    // if (data) {
    //     var date = getCurrentDateTime();
    //     var calendar = showCal();
    //     const birthdayInfo = `<div><p>今天是:   ${date}  农历： + ${calendar}</p><p>${birthday[0].name}</p></div>`;
...
function GetLunarDay(solarYear, solarMonth, solarDay) {
    //solarYear = solarYear<1900?(1900+solaryear):solaryear; 1921="" if="" (solaryear="" <="" ||="" solaryear=""> 2020) {
        return "";
    } else {
